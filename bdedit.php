...
if(isset($_POST['add_section'])){
    ...
    $conn->prepare("INSERT INTO sections (title, icon, link) VALUES (?, ?, ?)")->execute([$title, $icon, $link]);
...
if(isset($_POST['edit_section'])){
    ...
    $icon  = $_POST['icon'] ?? '';
    $link  = $_POST['link'] ?? null;
    $conn->prepare("UPDATE sections SET title=?, icon=?, link=? WHERE id=?")->execute([$title, $icon, $link, $id]);
    header("Location: admin_panel.php"); exit;
}
